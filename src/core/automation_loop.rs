                ...
                for item in &items {
                    ...
                    match &item.image_path {
                        Some(image_path) => {
                            ...
                            let full_path = Path::new(&base_path).join("image").join(image_path);
                            ...
                            if score <= setting.threshold {
                                for action in &item.actions {
                                    if action.enabled {
                                        ...
                                        match action.execute(top_hwnd, low_hwnd, &mut project, &setting, Some((x, y))) {
                                            true => {}
                                            false => break,
                                        }
                                    }
                        ...
                        None => {
                            for action in &item.actions {
                                if action.enabled {
                                    ...
                                    match action.execute(top_hwnd, low_hwnd, &mut project, &setting, Some((0, 0))) {
                                        true => {}
                                        false => break,
                                    }
                                }
