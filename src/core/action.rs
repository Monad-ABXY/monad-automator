...
use crate::{
    core::{
        expression::{Condition, Operand, VarOp},
    ...
    pub fn execute(
        ...
        match &self.action {
            ...
            ActionType::Drag { start, end, duration_ms } => {
                println!("{}", fl!("message-action-drag", x1 = start.0, y1 = start.1, x2 = end.0, y2 = end.1));
            ...
            ActionType::SendDiscord {
                ...
                if !use_screenshot {
                    ...
                    println!("{}", fl!("message-action-send-discord-with-screenshot", message = message.clone()));
            ...
            ActionType::ConditionCheck { condition } => {
                ...
                let cond_str = match condition {
                    Condition::Cmp { left, op, right } => fl!(
                        "action-display-condition-compare",
                        left = left.to_string(),
                        op = op.to_string(),
                        right = right.to_string()
                    ),
                    Condition::ImageMatch { target } => fl!("action-display-condition-image", target = target),
                    Condition::All(..) => "All(...)".to_string(),
                    Condition::Any(..) => "Any(...)".to_string(),
                    Condition::Not(..) => "Not(...)".to_string(),
                };

                match crate::expression::evaluate_condition(condition, project, hwnd_ptr, &setting) {
                    true => {
                        println!("{}", fl!("message-action-condition-check-success", condition = cond_str));
                        true
                    }
                    false => {
                        println!("{}", fl!("message-action-condition-check-fail", condition = cond_str));
                        false
                    }
                }
            }
            ActionType::VariableValueChange { name, op, value } => match crate::expression::evaluate_operand(project, value) {
                Some(rhs) => match crate::expression::apply_var_op(project, name, op, &rhs) {
                    Ok(new_val) => {
                        let val_str = match &new_val {
                            crate::core::VarValue::Bool(b) => b.to_string(),
                            crate::core::VarValue::Int(i) => i.to_string(),
                            crate::core::VarValue::Float(f) => f.to_string(),
                            crate::core::VarValue::Str(s) => s.clone(),
                        };

                        println!(
                            "{}",
                            fl!(
                                "message-action-var-change-success",
                                name = name,
                                op = op.to_string(),
                                value = val_str
                            )
                    ...
                    Err(err) => {
                        eprintln!("{}", fl!("message-action-var-change-fail", name = name, error = err));
                        false
                    }
                },
                None => {
                    eprintln!("{}", fl!("message-action-var-change-eval-fail", name = name));
                    false
                }
            },
        }
...
impl ActionType {
    pub fn to_localized_string(&self) -> String {
        match self {
            ActionType::LeftClick {
                ...
                if *use_matched_position {
                    format!(
                        ...
                        fl!("action-display-delay", millis = delay)
                    )
                } else {
                    format!(
                        "{} | {}",
            ...
            ActionType::ConditionCheck { condition } => match condition {
                Condition::Cmp { left, op, right } => fl!(
                    "action-display-condition-compare",
                    left = left.to_string(),
                    op = op.to_string(),
