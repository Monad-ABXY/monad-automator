...
use crate::{
    ...
    expression::{UserVariable, VarValue},
    fl,
...
pub struct Project {
    ...
    pub user_variables: Vec<UserVariable>,
    #[serde(skip)]
...
impl Project {
    ...
    pub fn get_variable_value(&self, name: &str) -> Option<VarValue> {
        self.user_variables
            .iter()
            .find(|v| v.name.eq_ignore_ascii_case(name))
            .and_then(|v| v.parse_self().ok())
    }
    ...
    pub fn load_from_json(path: PathBuf) -> Result<Self, String> {
        ...
        project.path = Some(path.parent().unwrap().to_string_lossy().to_string());
        Ok(project)
    }
